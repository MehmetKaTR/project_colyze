...
const MeasurementLog = ({ results, isHist }) => {
  if (!results.length) {
    return (
      ...
      <table className="table-auto w-full border-collapse border border-gray-300 text-gray-900 text-sm">
        <thead>
          <tr>
            ...
            {isHist ? (
              <>
                <th className="border border-gray-300 px-2 py-1">R_diff</th>
                <th className="border border-gray-300 px-2 py-1">G_diff</th>
                <th className="border border-gray-300 px-2 py-1">B_diff</th>
              </>
            ) : (
              <>
                ...
                {isHist ? (
                  <>
                    <td className={`border border-gray-300 px-2 py-1 text-center ${cellClass(res.Result)}`}>
                      {res.R_diff !== null ? res.R_diff : "-"}
                    </td>
                    <td className={`border border-gray-300 px-2 py-1 text-center ${cellClass(res.Result)}`}>
                      {res.G_diff !== null ? res.G_diff : "-"}
                    </td>
                    <td className={`border border-gray-300 px-2 py-1 text-center ${cellClass(res.Result)}`}>
                      {res.B_diff !== null ? res.B_diff : "-"}
                    </td>
                  </>
                ) : (
                  <>
